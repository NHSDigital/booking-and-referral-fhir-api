...
class TestMessageDefinition:
    ...
    @pytest.mark.sandbox
    def test_get_message_definition(self, get_token_client_credentials, base_url_path):
        # Given
        ...
        response = requests.get(
            url=f"{base_url_path}/MessageDefinition",
            headers={
    ...
    @pytest.mark.integration
    @pytest.mark.sandbox
    def test_message_definition_method_not_allowed(self, get_token_client_credentials, base_url_path):
        # Given
        token = get_token_client_credentials["access_token"]
