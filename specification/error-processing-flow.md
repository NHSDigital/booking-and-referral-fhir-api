...
stateDiagram-v2
    ...
    A3 --> 401_SEND_UNAUTHORIZED: Access Token not provided (code - unknown)
    ...
    A11 --> 404_PROXY_NOT_FOUND: The target endpoint does not exist in the Endpoint Catalogue (code - not-found)
    A12 --> 404_PROXY_NOT_FOUND: The Target Identifier found two endpoints (code - multiple-matches)
    A13 --> 500_PROXY_SERVER_ERROR: A sub-service encountered an unhandled exception (code - exception)
    ...
    A21 --> 403_REC_FORBIDDEN: TLS-MA failure (code - security)
    ...
    A23 --> 500_REC_SERVER_ERROR: unhandled exception (code - exception)
    A24 --> 500_REC_SERVER_ERROR: internal data storage issue (code - no-store)
